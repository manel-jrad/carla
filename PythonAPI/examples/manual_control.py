...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        mon_fichier = open("data/fichier12.txt", "a")
        now = dt.datetime.now()
        ns= now.second
        ts=str(ns)
        message = '%4.2f' % (100 * c.steer )
        message2='%2.0f' % (3.6 * math.sqrt(v.x**2 + v.y**2 + v.z**2))
        image.save_to_disk('data/image12/%6d' % image.frame_number)
        
        mon_fichier.write(message+','+str(image.frame_number)+','+message2+'\n')
